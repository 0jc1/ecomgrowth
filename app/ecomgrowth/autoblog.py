...
# graph API
# https://github.com/sns-sdks/python-facebook/tree/master
...
def gen_blog(api_key):
    ...
    payload = {
        "messages": [
            {
                "role": "user",
                "content": "You are a content writer expert in writing SEO-optimized blog posts. \
                Respond only with the blog post. Your output should NEVER contain your own output. \
                The blog is for a store called {shop_name}. \
                Write an SEO-optimized 100 word blog post with the SEO keyword {keyword}." \
                "ALWAYS write the title at the start with the format  Title: (Title here) "
            }
        ],
    ...
    if response.status_code == 200:
        ...
        # Loop through each line to find title and content
        for line in lines:
            # Check if the line starts with "Title:"
            if line.startswith("Title:"):
                title = line.replace("Title:", "").strip()
            # Assume content starts after the title and introduction
            else:
